...
int xml2rrd(char* buf, rrd_t* rrd){
  ...
  while (eat_tag(&ptr2,"ds") == 1){
      ...
      if((rrd->ds_def = realloc(rrd->ds_def,rrd->stat_head->ds_cnt*sizeof(ds_def_t)))==NULL){
	  rrd_set_error("allocating rrd.ds_def");
	  return -1;
      };
      /* clean out memmory to make sure no data gets stored from previous tasks */
      memset(&(rrd->ds_def[rrd->stat_head->ds_cnt-1]), 0, sizeof(ds_def_t));
      if((rrd->pdp_prep = realloc(rrd->pdp_prep,rrd->stat_head->ds_cnt
  ...
  while (eat_tag(&ptr2,"rra") == 1){
      ...
      if((rrd->rra_def = realloc(rrd->rra_def,rrd->stat_head->rra_cnt*sizeof(rra_def_t)))==NULL){
      ...
      if((rrd->cdp_prep = realloc(rrd->cdp_prep,
				  rrd->stat_head->rra_cnt
	  ...
	  if(mempool==0){
	    mempool = 1000;
	    if((rrd->rrd_value = realloc(rrd->rrd_value,
	  ...
	  for(i=0;i<rrd->stat_head->ds_cnt;i++){
	      read_tag(&ptr3,"v","%lf",&(rrd->rrd_value[(rows-1)*rrd->stat_head->ds_cnt+i]));
	  }
      	  eat_tag(&ptr3,"/row");                  
	  ptr2=ptr3;
      }
...
rrd_restore(int argc, char **argv) 
{
    ...
    rrd_init(&rrd);
    if (argc<3) {
	rrd_set_error("usage rrdtool %s file.xml file.rrd",argv[0]);
	return -1;
    }
    if (readfile(argv[1],&buf)==-1){
      return -1;
    }
    if (xml2rrd(buf,&rrd)==-1) {
	rrd_free(&rrd);
	free(buf);
	return -1;
    }
    free(buf);
    if(rrd_write(argv[2],&rrd)==-1){
	rrd_free(&rrd);	
	return -1;	
    };
    rrd_free(&rrd);    
