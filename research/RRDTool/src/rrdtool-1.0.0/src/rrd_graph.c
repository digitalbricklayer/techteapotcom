...
enum op_en {OP_NUMBER=0,OP_VARIABLE,OP_UNKN,OP_NOW,OP_TIME,OP_ADD,OP_SUB,OP_MUL,
	    OP_DIV,OP_SIN,
			   ...
			   isnan(im->gdes[i].p_data[ii]))
                    ...
                    if (im->gdes[i].gf == GF_STACK )
                        for(ii=0;ii<im->xsize;ii++){
                            if ((i>0 && isnan(lastgdes->p_data[ii])) ||
                                isnan(im->gdes[i].p_data[ii]))
                                continue;
                            gdImageLine(gif,
...
rrd_graph(int argc, char **argv, char ***prdata, int *xsize, int *ysize)
{
    ...
    image_desc_t   im;
    int            i;
    long           long_tmp,start_tmp=0,end_tmp=0;
    ...
    *ysize=im.ygif;
	...
	if((*prdata = calloc(2,sizeof(char *)))==NULL){
      ...
      if(((*prdata)[0] = malloc((200+strlen(im.graphfile))*sizeof(char)))
	 ==NULL){
	rrd_set_error("malloc imginfo");
	return -1;
      }
