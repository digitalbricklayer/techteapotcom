...
# -*- mode: Perl -*-
##################################################################
# MRTG 2.9.18  -- Config file creator
##################################################################
...
ECHO
	...
	my $comment_sysdescr = $sysdescr;
        my $router_connect = "$$router_ref{community}\@$$router_ref{routername}$$router_ref{snmpopt_current}";
	$comment_sysdescr =~ s/[\n\r]+/\n\#          /g;
...
sub IsCounterBroken ($$) {
    ...
    if ($router =~ /:\d*:\d*:\d*:\d*:2$/) {
            ...
            my $speed = (snmpget($router, 'ifHighSpeed.'.$if))[0];        
            debug('base',"snmpget $router for ifHighSpeed.$if -> $speed Mb/s");
            $SNMP_Session::errmsg = undef;
            my $counter = (snmpget($router, 'ifHCInOctets.'.$if))[0];
            debug('base',"snmpget $router for ifHCInOctets.$if -> $counter");
            if(not $speed or $counter eq "" or $SNMP_Session::errmsg){
                $SNMP_Session::errmsg = undef;
                $$router_ref{snmpopt_current} =~ s/:+2$//;
                debug('base',"check for HighspeedCounters faild ... Droping back to V1");
            } else {
...
cfgmaker - Creates mrtg.cfg files (for mrtg-2.9.18)
