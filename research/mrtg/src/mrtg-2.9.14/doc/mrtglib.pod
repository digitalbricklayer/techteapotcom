...
###################################################################
# MRTG 2.9.14  Support library MRTG_lib.pm
###################################################################
...
@EXPORT = qw(readcfg cfgcheck
	     datestr expistr ensureSL
             demonize_me debug log2rrd storeincache
...
sub readcfg ($$$$;$$) {
    ...
    while (1) {        
        ...
        if (/^include:\s*(.*?\S)\s*$/i){
                ...
                $nextfile = $1;
                local *FH;
                open (FH, $nextfile) || do { die "ERROR: unable to open include file: $nextfile\n"};
                $hand = *FH;
                next;
        }
    ...
    foreach $rou (@$routers) {
	    ...
	    $$rcfg{'directory_web'}{$rou} =~ s/\Q${MRTG_lib::SL}\E+/\//g;
     	...
     	if (! defined $$rcfg{"pagetop"}{$rou}) {
            warn ("WARNING: \"PageTop[$rou]\" is not specified.\n");
            $error = "yes";
        } else {
...
sub demonize_me ($) {
    ...
    if ( $OS eq 'NT' ) {
            ...
            $CONSOLE->Flush();
            $CONSOLE->Free();
            #Redirect errors and messages to log files
            open STDOUT,'>mtrgout.log';
            open STDERR,'>mtrgerr.log';
     } else {
...
sub debug ($$) {
    ...
    if ($main::debugfile and open DEB, ">>$main::debugfile"){
        print DEB "--",$tag,": ",$msg,"\n"
    } else {        
...
MRTG_lib is  Copyright 2000 by Tobias Oetiker E<lt>tobi@oetiker.chE<gt>
and Dave Rand E<lt>dlr@bungi.comE<gt>.
