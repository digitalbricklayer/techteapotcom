...
mrtg-2.9.24.zip or better. The archive does also contain a precompiled copy
...
Unzip MRTG to C:\mrtg-2.9.24 on the WindowsNT machine of your choice.
...
and go into F<c:\mrtg-2.9.24\bin>. Type:
...
Get to a cmd prompt and change to the F<c:\mrtg-2.9.24\bin> directory. Type the
following command:
...
results by going into the F<c:\mrtg-2.9.24\bin> directory and typing:
...
C<"c:\mrtg-2.9.24\bin"> after creating the task.  Select the user account
