...
unix-guide - Instructions for running MRTG 2.9.24 on a Unix Box
...
so that you can skip recompiling them. The B<wget> program used below is a
...
Ok now everything is ready for the mrtg compilation.
 ...
 cd /usr/local/src
 gunzip -c mrtg-2.9.24.tar.gz | tar xvf -
 cd mrtg-2.9.24
...
A B<minimal> script to put into F<init.d> might look like this:
 ...
 #! /bin/sh
 cd /usr/local/mrtg-2.9.24/bin && ./mrtg --user=mrtg-user \
