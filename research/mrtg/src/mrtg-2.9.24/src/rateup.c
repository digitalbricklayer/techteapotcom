...
#define xtr(x) (unsigned long)((growright) ? (maxx*xscale+81-((x)*xscale)) : (81+((x)*xscale))) 
	...
	if( inmax >0 || outmax > 0 || inr>0 || outr>0 || (options & OPTION_WITHZEROES)) valsamp++;
    ...
    printf("%lu\n",(unsigned long)(maxin/muli+.5));
    printf("%lu\n",(unsigned long)(maxout/muli+.5));
    if (options & OPTION_DORELPERCENT) {
       printf("%lu\n",(unsigned long)(maxpercent+.5));
    }
    printf("%lu\n",(unsigned long)(avin/muli+.5));
    printf("%lu\n",(unsigned long)(avout/muli+.5));
    if (options & OPTION_DORELPERCENT) {
       printf("%lu\n",(unsigned long)(avpercent+.5));
    }
    printf("%lu\n",(unsigned long)(latestin/muli+.5));
    printf("%lu\n",(unsigned long)(latestout/muli+.5));
    if (options & OPTION_DORELPERCENT) {
       printf("%lu\n",(unsigned long)(latestpercent+.5));
    }
    printf("%lu\n",(unsigned long)(avmxin/muli+.5));
    printf("%lu\n",(unsigned long)(avmxout/muli+.5));
...
/*  nmax=sca_max_q*ytics*(pow((double)kilo,(double)((int)(digits/3))));  */
    nmax=(sca_max_q/yticsf)*ytics*(pow((double)kilo,(double)((int)(digits/3))));
	...
	if ((unsigned long) y >= maxy) y = maxy;
	lhist[x].in = y;
...
*/
   ...
   if (!(options & OPTION_NO_O)) {
    ...
    if (maxy < (unsigned long) gdFontSmall->w*16) {	
...
static void update(in, out, abs_max, absupdate)
char *in, *out;
...
int absupdate;
{
	...
	/* gauge option */
	if (absupdate == 2) {
	    inrate = diff(in,last.in)+.5;
	    outrate = diff(out,last.out)+.5;
        } else if (absupdate == 3) {
            inrate=diff(in,last.in)*(3600.0/(period*1.0))+.5;
            outrate=diff(out,last.out)*(3600.0/(period*1.0))+.5;
        } else if (absupdate == 4)  {
            inrate=diff(in,last.in)*(60.0/(period*1.0))+.5;
            outrate=diff(out,last.out)*(60.0/(period*1.0))+.5;
	} else {
	    inrate = diff(in,last.in) / period + .5;
	    outrate = diff(out,last.out) / period + .5;
	}
    }
	...
	if (plannow < now) {
	  ...
	  fprintf(fo,"%lu %lu %lu %lu %lu\n",(unsigned long)now,
		  (unsigned long)inr,(unsigned long)outr,(unsigned long)inmax,(unsigned long)outmax);
	  hist->time = now;
...
static void init_colour(int *colmap, int c0, int c1, int c2) {
  *colmap++ = c0;
  *colmap++ = c1;
  *colmap = c2;
}
