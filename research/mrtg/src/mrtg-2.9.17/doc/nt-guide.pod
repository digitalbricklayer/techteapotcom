...
mrtg-2.9.17.zip or better. The archive does also contain a precompiled copy
...
and go into F<c:\mrtg-2.9.17\bin>. Type:
...
Get to a cmd prompt and change to the F<c:\mrtg-2.9.17\bin> directory. Type the
following command:
...
results by going into the F<c:\mrtg-2.9.17\bin> directory and typing:
