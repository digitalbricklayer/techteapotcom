...
http://people.ee.ethz.ch/~oetiker/webtools/mrtg/pub Look for mrtg-2.9.28.zip
...
and go into F<c:\mrtg-2.9.28\bin>. Type:
...
Get to a cmd prompt and change to the F<c:\mrtg-2.9.28\bin> directory. Type the
following command:
...
results by going into the F<c:\mrtg-2.9.28\bin> directory and typing:
