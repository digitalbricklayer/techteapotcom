...
ECHO
    ...
    $index .= "<tr><td>$$opt{headon}Interfaces of $host $$opt{headoff}</td></tr>" if $$opt{perhost};
               ...
               do{
                 if ($$rcfg{pagetop}{$item} =~
                     m,<TR><TD>Port Name:</TD>\s*<TD>([^<]+)</TD>,i ) {
                      ...
                      warn "ERROR: no Port Name: PageTop property in $item section\n";
                      $section = $item; last;
                 }
               };
...
sub options ($) {
    ...
    die ("Indexmaker for mrtg-2.9.27\n") if $$opt{version};
...
indexmaker - Creates index files for mrtg web sites (mrtg-2.9.27)
