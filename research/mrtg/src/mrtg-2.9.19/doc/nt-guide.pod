...
nt-guide - The Windows NT Guide to MRTG 2.9.19
...
mrtg-2.9.19.zip or better. The archive does also contain a precompiled copy
...
Unzip MRTG to C:\mrtg-2.9.19 on the WindowsNT machine of your choice.
...
and go into F<c:\mrtg-2.9.19\bin>. Type:
...
Get to a cmd prompt and change to the F<c:\mrtg-2.9.19\bin> directory. Type the
following command:
...
results by going into the F<c:\mrtg-2.9.19\bin> directory and typing:
...
to your mrtg.cfg file and start it with:
 ...
 start /Dc:\mrtg-2.9.19\bin wperl mrtg --logging=eventlog mrtg.cfg
...
C<"c:\mrtg-2.9.19\bin"> after creating the task.  Select the user account
